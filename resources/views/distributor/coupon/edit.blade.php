    ...
    <div class="card-body">
      <form method="post" action="{{route('coupon.update',$coupon->id)}}">
        ...
        <div class="form-group">
            <label for="dealer_id" class="col-form-label">Dealers  <span class="text-danger">*</span></label>
            <select name="dealer_id" class="form-control">
              <option value="">Select Dealer</option>
              @foreach ($dealers as $dealer_id => $dealer )
                <option {{(($coupon->dealer_id==$dealer_id) ? 'selected' : '')}} value="{{$dealer_id}}">{{$dealer }}</option>
              @endforeach
            </select>
            @error('dealer_id')
        ...
        <div class="form-group">
          ...
          <div class="form-group">
              ...
              <input id="value" type="text" name="value" placeholder="Enter Coupon value"  value="{{$coupon->value}}" class="form-control">
              @error('value')
              <span class="text-danger">{{$message}}</span>
              @enderror
          </div>
