...
@section('title','ROYALE TOUCHE || All Notifications')
...
<div class="card">
  ...
  <div class="card-body">
    @if(count(Auth::guard('distributor')->user()->Notifications)>0)
    <table class="table  table-hover admin-table" id="notification-dataTable">
      ...
      <tbody>
        @foreach ( Auth::guard('distributor')->user()->Notifications as $notification)
        ...
        <tr class="@if($notification->unread()) bg-light border-left-light @else border-left-success @endif">
          ...
          <td>
            <a href="{{route('distributor.notification', $notification->id) }}" class="btn btn-primary btn-sm float-left mr-1" style="height:30px; width:30px;border-radius:50%" data-toggle="tooltip" title="view" data-placement="bottom"><i class="fas fa-eye"></i></a>
