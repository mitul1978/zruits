 ...
 -->
  ...
  $.validate({
    ...
    setTimeout(function(){
        ...
        $("#arc_submit").on('click',function(){

                   var loc = window.location;
                   var baseURL = loc.protocol + "//" + loc.hostname;
                   var data = $("#user-info-form").serialize();
                   $.ajax({
                      url: baseURL+"/architectureform_submit",
                      type: 'post',
                      data:data,
                      beforeSubmit : function(){
                          $('#arc_submit').prop('disabled',true);
                          $('#arc_submit').val('Please wait...');

                      },
                      success: function(result) {
                        if(result == 'success'){
                              $('#arc_submit').val('Submit');
                          $("#user-info-form")[0].reset()
                           $('#arc_submit').prop('disabled',false);
                             $("#msg").html("Thank you for your details, our executive will get in touch with you shortly.");

                             setTimeout(function(){
                                $("#msg").html('');
                                $(".close-form-block").click();
                           }, 7000);
                        }
                        else if(result == 2){
                          $("#msg").html("We have received your contact details. Our team will get in touch with you!");
                             $('#arc_submit').val('Submit');
                          $("#user-info-form")[0].reset()
                           $('#arc_submit').prop('disabled',false);
                             setTimeout(function(){
...
//Functions for validation of mobile number.
